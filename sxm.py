...
class SiriusXM:
    ...
    def is_logged_in(self):
        return 'SXMAUTH' in self.session.cookies or 'SXMAKTOKEN' in self.session.cookies

    def is_session_authenticated(self):
        has_alb = any(k.startswith('AWSALB') or k.startswith('AWSELB') for k in self.session.cookies.keys())
        return self.is_logged_in() and (has_alb or 'JSESSIONID' in self.session.cookies)
    ...
    def login(self):
        ...
        if not data:
            self.log('Login request failed or returned empty payload')
            return False

        try:
            module_resp = data.get('ModuleListResponse', {})
            status = module_resp.get('status')
            messages = module_resp.get('messages', [])

            for msg in messages:
                self.log('Login Response Message: [{}] {}'.format(msg.get('code'), msg.get('message')))

            if status in (1, 100) or self.is_logged_in():
                self.log('Login successful')
                return True
            else:
                self.log('Login failed with status {}'.format(status))
                return False
        except Exception as e:
            self.log('Error parsing login response: {}'.format(e))
            return False
    ...
    def authenticate(self):
        ...
        try:
            status = data.get('ModuleListResponse', {}).get('status')
            return status == 1 or self.is_logged_in()
        except Exception:
            self.log('Error parsing response for session refresh')
            return self.is_logged_in()

    def get_sxmak_token(self):
        try:
            if 'SXMAKTOKEN' in self.session.cookies:
    ...
    def get_gup_id(self):
        try:
            if 'SXMDATA' in self.session.cookies:
    ...
    def get_channel(self, name):
        ...
        for x in self.get_channels():
            if (x.get('name', '').lower() == name or
                ...
                str(x.get('siriusChannelNumber', '')).lower() == name):
